...
import { incrementPropertyViews } from "@/lib/firebase";
...
export default function PropertyCard({
  ...
  const handlePropertyClick = async () => {
    try {
      console.log("Clic sur la propriété avec l'ID:", property.id);
      // Incrémenter les vues
      await incrementPropertyViews(property.id);
    } catch (error) {
      console.error("Erreur lors de l'incrémentation des vues:", error);
      // On ne bloque pas la navigation même si l'incrémentation échoue
    }
  };
    ...
    >
      <div className="relative overflow-hidden h-64">
        ...
        <div className="absolute top-4 left-4">
          <span
            className={`inline-block px-3 py-1 text-xs font-medium ${statusColor} text-white rounded-full`}
          >
            {statusLabel}
          </span>
        </div>
      ...
      <div className="px-6 pb-6">
        <Link
          ...
          className="block text-center py-3 px-4 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-800 dark:text-white font-medium rounded-lg transition-colors duration-300"
        >
          Voir les détails
        </Link>
      </div>
