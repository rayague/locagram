...
import { getFirestore, connectFirestoreEmulator, collection, getDocs, query, where, orderBy, limit, deleteDoc, doc, setDoc, writeBatch, getDoc } from "firebase/firestore";
...
export const resetCategories = async () => {
  try {
    ...
    existingSnapshot.docs.forEach((doc) => {
      console.log(`Suppression de la catégorie: ${doc.id} - ${doc.data().name}`);
    ...
    if (finalSnapshot.size !== EXACT_CATEGORIES.length) {
      throw new Error(`Nombre incorrect de catégories: ${finalSnapshot.size} au lieu de ${EXACT_CATEGORIES.length}`);
    }
...
export const getProperties = async (filters: {
  ...
  try {
    ...
    snapshot.docs.forEach(doc => {
      const data = doc.data();
      console.log(`- ID: ${doc.id}, Catégorie: ${data.category}, Titre: ${data.title}`);
    });
    ...
    const results = snapshot.docs
      .map(doc => {
        ...
        console.log('Détails de l\'annonce:', {
          ...
          categoryMatches: filters.category ? property.category === filters.category : 'N/A'
        });
      ...
      .filter(property => {
        ...
        if (filters.category) {
          console.log(`Filtrage pour ${property.id}:`, {
            ...
            matchesType,
            matchesPrice,
            finalResult: matchesLocation && matchesType && matchesPrice && matchesCategory
          });
        }
    ...
    if (filters.category) {
      console.log('Annonces filtrées par catégorie:', results.map(p => ({
        id: p.id,
        category: p.category,
        title: p.title
      })));
